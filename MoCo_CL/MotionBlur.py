...
class RandomMotionBlur(object):
    ...
    def __call__(self, Image, intensity=None):
        if not intensity:
            intensity = random.uniform(0.5, 1)
        motion_blur_kernel = Kernel(size = self.size, intensity=intensity)
        img = motion_blur_kernel.applyTo(Image, self.size, keep_image_dim=True)
        return img
...
class Kernel(object):
    ...
    def __init__(self, size: tuple = (100, 100), intensity: float=0):
        ...
        self.PAD = transforms.Pad(size[0], padding_mode='edge')
    ...
    def applyTo(self, image, size: int,keep_image_dim: bool = False) -> Image:
        """[summary]
        ...
        if isinstance(image, str) or isinstance(image, Path):
            ...
            # print("assdfasfasfas"*10, flush=True)
            image = applyToPIL(image, keep_image_dim)
            # print
